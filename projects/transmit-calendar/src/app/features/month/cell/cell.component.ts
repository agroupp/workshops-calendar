...
import { AddEventComponent, ViewEventComponent, EventsListComponent } from '../../dialogs';
...
export class CellComponent implements OnInit, OnChanges {
  ...
  constructor(private dialog: MatDialog, private store: Store<MonthState>, private snackBar: MatSnackBar) { }
  ...
  ngOnChanges() {
    ...
    this.day.events.sort((a, b) => a.dateStart.getTime() - b.dateStart.getTime());
  }
  ...
  addEvent(): void {
    if (this.day.events.length === 5) {
      this.snackBar.open('You can have 5 events per day maximum', 'Add Event', {duration: 2000});
      return;
    }
