...
export class EventsService {
  ...
  getEvents(month: number, year: number): Observable<IEvent[]> {
    return this.getAllEvents().pipe(
      map(events => events.filter(e => e.dateStart.getFullYear() === year && e.dateStart.getMonth() === month - 1))
    );
  }

  getAllEvents(): Observable<IEvent[]> {
    return of(this.ls.collectionGet<IEvent>(COLLECTION_NAME)).pipe(
      map(events => events.map(event => ({...event, dateStart: new Date(event.dateStart), dateEnd: new Date(event.dateEnd)})))
    );
  }
