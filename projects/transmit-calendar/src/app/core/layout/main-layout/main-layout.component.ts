...
export class MainLayoutComponent implements OnInit {
  ...
  onNextMonthClick(): void {
    ...
    const year = this.currentMonth < 12 ? this.currentYear : this.currentYear + 1;
  ...
  onPrevMonthClick(): void {
    ...
    const year = this.currentMonth > 1 ? this.currentYear : this.currentYear - 1;
    const url = `/month/${year}/${prevMonth}`;
    this.router.navigate([url]);
  }

  onTodayClick(): void {
    const url = `/month/${this.today.getFullYear()}/${this.today.getMonth() + 1}`;
    this.router.navigate([url]);
  }
}
