...
export class EventsEffects {
  ...
  createEvent$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap(action => this.eventsService.add(action.event).pipe(
      map(event => fromEventsActions.addEventSuccess({ event })),
      catchError(err => EMPTY)
    ))
  ));

  removeEvent$ = createEffect(() => this.actions$.pipe(
    ofType(fromEventsActions.removeEvent),
    map(action => action.id),
    mergeMap(id => this.eventsService.delete(id).pipe(
      map(() => fromEventsActions.removeEventSuccess({id})),
      catchError(error => of(fromEventsActions.removeEventFailure({ error })))
    ))
  ));
  ...
  loadEvents$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap(params => this.eventsService.getEvents(+params.month, +params.year).pipe(
      map(events => fromEventsActions.loadMonthEventsSuccess({ events })),
      catchError(error => of(fromEventsActions.addEventFailure({ error })))
    ))
  ));
